import Robot from "../../WalkingRobot/components/Robot"
import useGetFrame from "../../../hooks/useGetFrame"
...
const AgentEnvironment:React.FC<Props> = ({isLoading}) => {
  ...
  const [viewBox, setViewBox] = useState('0 0 10 10');

  useEffect(() => {
    if (frame) {
      const robotX = frame.hull.coordinate[0];
      const viewBoxX = xScale(robotX - 5);
      setViewBox(`${viewBoxX} 0 10 10`);
    }
  }, [frame, xScale, yScale]);

  return (
    <div className="agentEnvironmentContainer">
        ...
        {(isLoading || !frame) ? (
          ...
          <>
            ...
            <button 
              ...
              className={`${AgentEnvironmentStyles.settingsButton} circularButton`}
              aria-label="settings"
            >
            ...
            <div className={AgentEnvironmentStyles.extraInfoContainer}>
              <EnvironmentMinimap terrainPath={minimapPath} hullCoordinate={frame.hull.coordinate} distanceTraveled={distanceTraveled}/>
            </div>
